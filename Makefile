...
BUILD_WATCHER_CDK_ENV ?= SERVICE=$(SERVICE)-watcher ENV=$(ENV) DOMAIN=$(WATCHER_DOMAIN) ACM_ID=$(WATCHER_ACM_ID) HOSTED_ZONE_ID=$(HOSTED_ZONE_ID) WORKSPACE_ROOT=$(WORKSPACE_ROOT) CODE_PATH=$(PWD)/.build/watcher ENABLE_DYNAMO=false
